...
df <- read.csv("data/csv/germination_timetoevent_Pinus_sylvestris.csv")
...
saveRDS(GRlist,file="output/rds/GR_mod1_Pinus_sylvestris.rds")
...
# GRlist <- readRDS("output/rds/GR_mod1_Pinus_sylvestris.rds")

GRlist$ID <- paste(GRlist$serial_number, GRlist$treat, GRlist$temp, GRlist$dish, sep = "_")

# Removing outliers results
GRlist = subset(GRlist, !(serial_number == "912563" & temp == 15))
GRlist = subset(GRlist, ID != "912563_COLD_20_1")
GRlist = subset(GRlist, ID != "945534_NS_15_4")
...
write.csv(Out_w, "output/csv/Thresholds_models_Pinus_sylvestris.csv", row.names = FALSE)
write.csv(Out, "output/csv/results_models_Pinus_sylvestris.csv", row.names = FALSE)
write.csv(Predictions, "output/csv/predictions_Pinus_sylvestris.csv", row.names = FALSE)
...
GRlist_group_med <- GRlist_group_med %>% arrange(Altitude)
...
# Arranging the dataframe by lat_dd in ascending order
Predictions <- Predictions %>% arrange(Altitude)
...
unique(Predictions[,c("serial_number", "Altitude", "Geographical.Location")])

loc_labels <- Predictions %>%
  group_by(serial_number) %>%
  summarise(Geographical.Location = unique(Geographical.Location),
            Altitude = unique(Altitude)) %>%
  mutate(Geographical.Location = paste(sub(" - ", "\n", Geographical.Location),"\n", Altitude,"m")) %>% #serial_number, "\n",
...
# Arranging the dataframe by lat_dd in ascending order
df_To1 <- df_To1 %>% arrange(Altitude)
...
#max_pred = max(GRlist_group_med1$Median) + 0.1
...
save(plot, file = "output/rda/plot_pinus.rda")
